...
void ObjectMgr::LoadZoneLevels()
{
    uint32 oldMSTime = getMSTime();
    //                                                0     1
    QueryResult result = WorldDatabase.Query("SELECT zone, level FROM zone_levels");

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 zone levels. DB table `zone_levels` is empty!");
        return;
    }
    ...
    do
    {
        ...
        _levelzones[zone] = level;
        ++count;
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u zone levels in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}

uint32 ObjectMgr::GetLevelZone(uint32 zone)
{
    LevelZones::const_iterator itr = _levelzones.find(zone);
    if (itr != _levelzones.end())
        return itr->second;
    return NULL;
}