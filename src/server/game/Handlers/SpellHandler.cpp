...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    {	
		...
		*/
		//Let's try to do some ugly hardcode Fractional
		struct DisplayHelper {
			static int pickRace()
			{
				int races[9] = {1, 2, 3, 4, 6, 7, 8, 10, 11};
				int rIndex = rand() % sizeof(races) / sizeof(races[0]);
				return races[rIndex];
			}
			...
			static int pickDisplay(int race, int gender)
			{
				//One array per race, after gen mod 0 = male, 1 = female
				//IDs are defined in ChrRaces.dbc maleModel & femaleModel columns
				int displays[9][2] = {
					{49, 50}, //Human
					{51, 52}, //Orc
					{53, 54}, //Dwarf
					...
					{1563, 1564}, //Gnome
					{1478, 1479}, //Troll
					{15476, 15475}, //BloodElf
					{16125, 16126} //Draenei
				};

				return displays[race][gender - 1];
			}
		};
		...
		if (outfit < 0 && display == sObjectMgr->GetCreatureDisplay(outfit))
		{
			...
			if (it != outfits->end())
			{
				...
				uint8 race = DisplayHelper::pickRace();
				uint8 gender = DisplayHelper::pickGender();
				//uint32 display = DisplayHelper::pickDisplay(race, gender);
				uint8 skin = rand() % 2 + 1;
				uint8 face = rand() % 6 + 1;
				uint8 hair = rand() % 6 + 1;
				uint8 hairColor = rand() % 2 + 1;

				data << uint64(guid);
				data << uint32(display); //Only works as long as it is 50               // displayId
				data << uint8(7);         // race
				data << uint8(1);       // gender
				data << uint8(it->second.Class);        // class
				data << uint8(skin);         // skin
				data << uint8(face);         // face
				data << uint8(hair);         // hair
				data << uint8(hairColor);    // haircolor
				data << uint8(it->second.facialhair);   // facialhair
				data << uint32(0);                      // guildId
