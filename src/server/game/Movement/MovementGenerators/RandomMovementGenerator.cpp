...
void RandomMovementGenerator<Creature>::_setRandomLocation(Creature* creature)
{
    ...
    //init.MoveTo(destX, destY, destZ, true); //Fractional (added true)
    //init.SetWalk(true);
    //init.Launach();
	if (i_path)
		delete i_path;

	i_path = new PathGenerator(creature); //Fractional START
...
bool RandomMovementGenerator<Creature>::DoUpdate(Creature* creature, const uint32 diff)
{
	...
	if (i_recalculateTravel) //Fractional
		_setRandomLocation(creature);
