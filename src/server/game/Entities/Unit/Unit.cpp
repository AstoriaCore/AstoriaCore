...
#include "../../../scripts/Custom/MobRegulator.h"
#include "../../../scripts/Custom/MapRegulator.h"
#ifdef ELUNA
#include "LuaEngine.h"
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
	...
	{
		bool shouldModify = (GetTypeId() != TYPEID_PLAYER && (GetOwner() && GetOwner()->GetTypeId() != TYPEID_PLAYER));
		if (shouldModify)
		{
			sSpellRegulator->Regulate(damage, spellProto->Id);
			sMapRegulator->RegulateSpellDamage(damage, this->GetMapId());
		}
	}
...
uint32 Unit::CalculateDamage(WeaponAttackType attType, bool normalized, bool addTotalPct)
{
	...
	uint32 damage = urand(uint32(minDamage), uint32(maxDamage));

	bool shouldAlterDamage = GetTypeId() != TYPEID_PLAYER;

	if (GetOwner())
		if (GetOwner()->GetTypeId() == TYPEID_PLAYER)
			shouldAlterDamage = false;

	if (shouldAlterDamage)
	{
			sMobRegulator->RegulateDamage(damage, this->GetEntry());
			sMapRegulator->RegulateDamage(damage, this->GetMapId());
	}
...
void Unit::SendSpellNonMeleeDamageLog(SpellNonMeleeDamage* log)
{
	...
	if (shouldAlterDamage)
	{
		...
		sMapRegulator->RegulateSpellDamage(log->damage, this->GetMapId());
	}
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
	...
	if (shouldAlterDamage)
	{
		...
		sMapRegulator->RegulateSpellDamage(pInfo->damage, this->GetMapId());
	}
...
uint32 Unit::GetModelForTotem(PlayerTotemType totemType)
{
    switch (getRace())
    {
		case RACE_UNDEAD_PLAYER: //Fractional
		case RACE_BLOODELF:
        case RACE_ORC:
        {
            switch (totemType)
