...
#include "TradeData.h"
#include "../../../scripts/Custom/CustomSpellSystem.h" //Fractional
...
void Spell::cast(bool skipCheck)
{
    ...
    if (Player* playerCaster = m_caster->ToPlayer())
    {
		...
		if (sCustomSpellSystem->isSpellPartOfSystem(this->GetSpellInfo()->Id)) //Fractional
		{
			if (playerCaster->GetCustomSpellAmount() > sCustomSpellSystem->getMaxSpells())
			{
				playerCaster->GetSession()->SendAreaTriggerMessage("Your mind is confused. You know too many spells.");
				cancel();
				return;
			}
		}
...
void Spell::SendSpellStart()
{
    ...
    if (((IsTriggered() && !m_spellInfo->IsAutoRepeatRangedSpell()) || m_triggeredByAuraSpell) && !m_cast_count)
		if (!_triggeredCastFlags == TRIGGERED_IGNORE_POWER_AND_REAGENT_COST) //Fractional REWRITE
